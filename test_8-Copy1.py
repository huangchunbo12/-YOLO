...
# ✅ 目标YOLOv8n权重路径（重点监控）
YOLO8N_WEIGHT_PATH = (
    "/root/autodl-tmp/yolo/ultralytics/ultralytics-main/runs/detect/runs/detect/SOTA_YOLO8n/weights/best.pt"
)
MAX_PARALLEL = 3
...
if __name__ == "__main__":
    ...
    print("🚀 开始生成测试集全量可视化结果（含YOLOv8n训练后模型）")
    print("==================================================")
    ...
    if not TEST_IMAGES_DIR.exists():
        ...
        exit(1)
    image_count = len(
        [f for f in TEST_IMAGES_DIR.iterdir() if f.is_file() and f.suffix.lower() in [".jpg", "png", "jpeg", "bmp"]]
    )
    if image_count == 0:
