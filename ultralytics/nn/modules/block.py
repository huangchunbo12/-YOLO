...
class AAttn(nn.Module):
    ...
    def __init__(self, c1, num_heads, area=1):
...
class SPDConv(nn.Module):
    ...
    def __init__(self, c1, c2, dimension=1):
        super().__init__()
        self.d = dimension
        self.conv = nn.Conv2d(c1 * 4, c2, 1, 1, bias=False)
...
class LSKBlock(nn.Module):
    ...
    def __init__(self, c1, c2):
        ...
        self.conv_final = nn.Conv2d(dim // 2, dim, 1)
    ...
    def forward(self, x):
        ...
        attn = self.conv_squeeze(torch.cat([avg_attn, max_attn], dim=1)).sigmoid()
...
class BRA(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(lambda t: t.view(B, self.num_heads, C // self.num_heads, -1), qkv)
        attn = (q.transpose(-2, -1) @ k) * self.scale
        attn = attn.softmax(dim=-1)
        attn = torch.where(attn > self.gate_threshold, attn, torch.zeros_like(attn))
